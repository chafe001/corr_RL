...
% VERSION HISTORY
% v1: modification of TOPX recording task.  Reworked conditionsSetup to
% generate arrays with randomized stimulus position and identity for cue
% sequences.  

% v2: Streamlining code by removing most of the TOPX training stuff.

% ---------------- EXPERIMENTAL DESIGN ------------------
% This is an experiment to evaluate how learning modifies prefrontal
% representations.  It will test the interaction between associative and
% reinforcement learning at behavioral and neural levels implicated by
% their interdependence at the level of dendritic spines in slice
% experiments. It will also test the role of sequence prediction error
% minimization in governing the plasticity of prefrontal representations.
% There are two experiments, employed cue pairs and cue sequences.

% CUE PAIRS:  Intention is to engage STDP to vary neural correlations
% during experiment, read out synaptic plasticity
% Each cue (A and B) is an association learned between sets of 2 
% peripheral gabors. Select 4 gabor orientations and locations at random:
% A, B, C, D. 
% COMBINATIONS:
% A-cues: A-B, C-D
% B-cues: A-C, B-D
% Each individual gabor is associated with A and B-cues with 50%
% probability.  Correct performance demonstrates learned association
% between 2 stimuli.  Individual stimuli do not provide information about
% state (A vs B).  
% TIMING: (-> denotes 'precedes')
% A-cues: A->B, D->C
% B-cues: C->A, B->D
% Probe subject with single gabors. In the above stimulus orderings, A
% implies B, or A-cue.  If subjects have learned the association, A
% presented alone should invoke A-cue state rather than B-cue state, even
% though A is associated with A-cue and B-cue states with equal
% probability. Potential confound: an explicit cortical representation of 
% sequence (A-cue if A is the first gabor shown) would manifest the same 
% way in behavior.
% This is intrinsically a categorization experiment, so abstraction is
% required.  No probe stimulus in this case. Stim sequence, then button
% press.  Vary reward prediction error on button press to engage 
% reinforcment learning and investigate the interaction between STDP and 
% RL.  To increase stimulus repetitions, present each stimulus pair
% multiple times per trial (maybe 20 pairs). Hebbian plasticity is slow.

% RESPONSE: Press a response key as soon as the response cue appears.
% Contrast response times when response cue appears at high probability
% or low probability locations given the state.  Use two high and two low
% probability locations so neural correlates of state do not equate to
% individual response locations. Alternatively, can show two pairs, match
% to sample on category.

% CUE SEQUENCES: Cue-probe combinations require left/right responses as in
% TOPX.  Vary cue-probe probabilities and timing to manipulate sequence
% prediction error.  Relate sequence prediction error to changes in
% performance, neural signals, and neural correlations. DONT CHANGE
% RESPONSE RULE FROM TOPX. AX left, other cue-probe combinations right.
% Will facilitate collection of preliminary data.
...
if TrialRecord.CurrentTrialNumber == 1
    % --- seqPred_setCond assigns stimulus combinations to condition numbers
    [condAra, condAraLabel, trialAra, numConds] = seqPred_setCond(trialSet);
...
cue_eyejoy_holdTime = 1500;              % CUE DURATION: Time required to hold eye and joystick fixation at center while cue appears. % was 1000, needs to be 250?
...
% --- CUE EVENT LOGGING SHOULD WORK FOR seqPred, fewer stimulus types,
% won't explicity encode cue number, but recover from sequence
% cueOnEventCode = 20-21, controlled by condition number below
cueOff_eventCode = 30;
% probeOnEventCode = 40-41, controlled by condition number below
...
% BYPASSING CODE BELOW, will use stimulus information in condAra instead...

% This switch statement assigns stimulus variables to trial conditions
% according to how the conditions are specified in the conditions txt file.
% Refer there so understand the structure.
...
cueSeq = ImageChanger(null_);
% Read the trial condition number from TrialRecord
c = TrialRecord.CurrentCondition;
% size applied to cueStim belows returns [1 n], where 2nd dimension is number of
% stimuli in this cue sequence
numCues = size(TrialRecord.User.Condara(c).cueStim, 2); 
% have to extract and hard code stimulus information from condAra into List
% for imagechanger, as it depends on the number of stimuli shown.
% Providing for 1-4 now.
switch numcues
