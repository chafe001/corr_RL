...
naxes_to_scale = 2; % scale to largest axis (1) or both axes (2)
% center_by = 'center_of_mass';
center_by = 'simple_center';

lim = 1.0; % shapes scaled to go from roughly -0.5 to 0.5
...
curveParams.plotCurveSeq = false;
...
for b = 1:curveParams.nBlocks
    ...
    if curveParams.save_png
        cd('D:\code\tasks\corr_RL\corr_RL task code\blockstim')
        ...
        if curveParams.plotCurveSeq
            ...
            for ec = 1: length(curveParams.endcurve_t)
                for oc = 1: length(curveParams.orthocurve_t)
                    ...
                    y = squeeze(y);
                    [x, y] = centerandscale([x y], naxes_to_scale, center_by);
                    plot(x,y);
                    xlim(limx);
                    ylim(limy);
                    axis off;
...
function [x y] = centerandscale(incurve, naxes_to_scale, center_by)
% I wish I'd taken linear algebra . . .

xrange = max(incurve(:,1)) - min(incurve(:,1));
yrange = max(incurve(:,2)) - min(incurve(:,2));
scalecurve(:,1) = incurve(:,1) - min(incurve(:,1));
scalecurve(:,2) = incurve(:,2) - min(incurve(:,2));

if (naxes_to_scale == 1) % scale to largest axis
    scalefactor = max([xrange yrange]);
    scalecurve = scalecurve/scalefactor;
else
    scalecurve(:,1) = scalecurve(:,1)/xrange;
    scalecurve(:,2) = scalecurve(:,2)/yrange;
end

if (strcmp(center_by, 'center_of_mass'))
    % *** find center of mass of the curve ***
    dx = diff(scalecurve(:,1));
    dy = diff(scalecurve(:,2));
    ...
    x_mid = (scalecurve(1:end-1,1) + scalecurve(2:end,1)) / 2;
    y_mid = (scalecurve(1:end-1,2) + scalecurve(2:end,2)) / 2;
    
    % Calculate center of mass
    cm_x = sum(x_mid .* segment_lengths) / total_length;
    cm_y = sum(y_mid .* segment_lengths) / total_length;
    
    % center by subtracting com
    x = scalecurve(:,1) - cm_x;
    y = scalecurve(:,2) - cm_y;
else
    x = scalecurve(:,1) - max(scalecurve(:,1))/2;
    y = scalecurve(:,2) - max(scalecurve(:,2))/2;
end
end % centerandscale()
