...
% --- v2:
% - adding keypress rather than joystick for human subjects
% - adding capability to present choice array before and during cue movie
...
rew.duration = 200;
rew.numDrops = 1;
% choiceTiming controls when choice array appears
choiceTiming = 'duringCueMovie';
% choiceTiming = 'afterCueMovie';
...
times.sc2_wait = 0;
times.sc2_hold = 4000;
% scene 2 can also present movie and choices concurrently, in which case
times.sc2_mt_movie_wait = 6000; % make sure longer than movie sequence
times.sc2_mt_movie_hold = 300;
% scene 3 requires eye and joystick fixation wth
times.sc3_wait = 0;
times.sc3_hold = 3000;
% scene 3 timing is controlled by MultiTarget() joystick response
times.sc3_mt_wait = 15000;
...
% --- BUILD ADAPTOR CHAINS
% Choices can be displayed during movie if desired.  Create 2 alterative
% adaptor chains, one with and one without MultiTarget(), then select which
% of these to AND with movie at last step of scene construction
...
switch choiceTiming
    ...
    case 'duringCueMovie'
        ...
        sc2_mTarg_joy.setTarget(sc2_img);
        sc2_mTarg_joy.Threshold = joy_radius;
        sc2_mTarg_joy.WaitTime = times.sc2_mt_movie_wait;
        sc2_mTarg_joy.HoldTime = times.sc2_mt_movie_hold;
        sc2_mTarg_joy.TurnOffUnchosen = false;

        % Mark behavioral event: probeResp, when MultiTarget choice is acquired
        sc2_mTarg_joy_oom = OnOffMarker(sc2_mTarg_joy);
        sc2_mTarg_joy_oom.OffMarker = codes.choiceMade;
        ...
        sc2_resp = AllContinue(sc2_mTarg_joy_oom); % this will hopefully end scene when joy moves
        sc2_resp.add(sc2_wtHold);
        ...
        % time but scene timing will depend on MultiTarget, if this is the first
        % adaptor added.
        sc2_wtHold_movie = Concurrent(sc2_mTarg_joy_oom);
        ...
        % --- CHECK BEHAVIORAL OUTCOMES
        if sc2_mTarg_joy.Success % response made to one of two response windows, not nessarily the correct one
            if sc2_mTarg_joy.ChosenTarget == 1 % red target
                eventmarker(codes.chose1);
            elseif sc2_mTarg_joy.ChosenTarget == 2 % incorrect target
    ...
    case 'afterCueMovie'
        ...
        % time but scene timing will depend on movie only, if this is the first
        % adaptor added.
        sc2_wtHold_movie = Concurrent(sc2_movie);
        sc2_wtHold_movie.add(sc2_wtHold);
...
% only execute scene 3 if choice timing is after movie
if strcmp(choiceTiming, 'afterCueMovie')
    % --- BUILD ADAPTOR CHAINS
    % Present fixation target and establish XY position window for eye
...
switch choiceTiming
    case 'duringCueMovie'
        ...
        elseif sc2_mTarg_joy.ChosenTarget == 2 % green target
