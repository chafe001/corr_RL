% TOPX Task for Recording

% --- VERSION NOTES
% v3_5: 
% DONE
% - Sam added L/R differential reward nums to counteract biases (from V3)
% - Random number of drops toggle added
% - Fake ITI scene to add time for manual training
% - add condAra to bvh output file
% - removed TrialRecord from TOPXconditionsSetup function - now we will
% just save all the info it generates to TrialRecord here (no messing with
% the variable itself!)
% - 300 trial stack created and tested

% v3_6:
% - provide unique event codes for different cues/probes, and responses
% - add pretrial and posttrial states for bracketing data collection
% - initially broke monkeyLogic, note to self, don't include a keyword
% ('return') in a variable name.  Bug fixed, runs without crashing, new
% event logging is working.

% v4_0:
% - prerecording clean-up
% - check event logging in both task code and analysis c
% - streamline adaptor chains, particularly around probe/response
% - confirm event codes are making it to the NI DAQ digital IO
% - wire junction box to connect ML and SpikeGadgets computers
% - confirm event codes are making it to SpikeGadgets
% - record dummy files with monkey running the task and signal generator
% producing fake spikes
...
if TrialRecord.CurrentCondition == 1 %AX trial
    % --- set coordinates for MultiTarget response windows
...
multipleHits = false;
numHits = 4;
rewCorrect = false;  % dispenses 1 small drop of juice with every correct response, but keeps the "jackpot" for MH
...
% Options: 'prep_250', 'cond36_trials480', 'cond36_trials400', 'cond36_trials300',
...
if TrialRecord.CurrentTrialNumber == 1
    [condAra, condAraLabel, trialAra, numConds] = TOPXconditionsSetup(trialSet);
...
elseif rndNumDrops
    ...
    normRewDur = 200;   % 220/235 by 5
    normNumDrops = 2;   % 140 by 2
    smRewDur = 65;
    smNumDrops = 1;
end
...
if toneFeedback
    ...
    elseif sc4_mTarg_joy.ChosenTarget == 2 % incorrectTarg
        sc6_feedbackSnd = AudioSound(null_);
        sc6_feedbackSnd.List = 'load_waveform({''sin'', 0.5, 500})';  % 500 Hz sine wave for 0.5 sec
    end
end
