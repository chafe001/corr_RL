...
function [conditions, trials, params, stimPos, stimImage] = HebbRL_buildTrials_noChoiceTarg()
...
params.truePairBase = 0.7;  % proportion of maxPairReps that are true pairs
params.truePairInc = 0.2;  % proportion of maxPairReps to add for this block
...
for b = 1 : params.numBlocks
    ...
    if ~params.noisePairs
        blockPairReps = params.maxPairReps;
    else % introduce noise pairs
        minReps = floor(params.maxPairReps * params.truePairBase);
        plusReps = floor(params.maxPairReps * params.truePairInc);
        blockPairReps = minReps + randi(plusReps);
    end
    ...
    conditions(b, 2).condNum = ((b-1) * 8) + 3;
    ...
    conditions(b, 3).condNum = ((b-1) * 8) + 5;
    ...
    conditions(b, 4).condNum = ((b-1) * 8) + 7;
