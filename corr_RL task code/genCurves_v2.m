...
params.nBlocks = 10;
params.nStates = 2;
params.nMoviesPerState = 50;
params.nCurvesPerMovie = 10;
...
params.N = 50; % number of samples per exposure
...
for b = 1:params.nBlocks
    ...
    for s = 1:params.nStates  % 2 states per block (L/R response)
        ...
        % establish vector of t values corresponding to individual curves
        % in range for this state
        t = interp1([1, params.N], [lower_t_lim, upper_t_lim], 1:params.N);


        for m = 1:params.nMoviesPerState % number distinct movies, sections of manifold, per state



            for c = 1:params.nCurvesPerMovie
